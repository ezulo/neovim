...
-- Folding
vim.opt.foldcolumn = '1' -- '0' is also fine
...
-- LSP
vim.keymap.set("n", "<leader>lsp", ":lua =vim.lsp.get_clients()<CR>", { desc = "Get active clients" })
vim.keymap.set("n", "<leader>af", vim.lsp.buf.format, { desc = "Manual LSP format" })
...
local afgroup = vim.api.nvim_create_augroup("Autoformat", {})
local shlgroup = vim.api.nvim_create_augroup("SearchHighlighting", {})
...
vim.keymap.set("t", "<leader>tt", CloseFloatingTerminal,
  { noremap = true, silent = true, desc = "Close floating terminal from terminal mode" })
vim.keymap.set("t", "<Esc><Esc>", CloseFloatingTerminal,
  { noremap = true, silent = true, desc = "Close floating terminal from terminal mode" })
...
vim.keymap.set('n', '<leader>tj', ':tabnext+<CR>', { desc = 'Navigate one tab right' })
vim.keymap.set('n', '<leader>tk', ':tabnext-<CR>', { desc = 'Navigate one tab left' })
...
local function update_mode_color()
  ...
  local colors = "guibg=lightgrey guifg=black"
  if current_mode == "n" then
    colors = colors
  elseif current_mode == "i" or current_mode == "ic" then
    colors = "guibg=darkblue guifg=white"
  elseif current_mode == "v" or current_mode == "V" or current_mode == '\x16' then
    colors = "guibg=magenta guifg=black"
  elseif current_mode == "R" then
    colors = "guibg=cyan guifg=black"
  elseif current_mode == "c" then
    colors = "guibg=black guifg=white"
  elseif current_mode == "t" then
    colors = colors
  end
  vim.api.nvim_exec2("hi StatusLine " .. colors, { output = false })
  vim.api.nvim_exec2("hi StatusLineBold " .. colors, { output = false })
end
local statusgroup = vim.api.nvim_create_augroup("StatusLine", {})
vim.api.nvim_create_autocmd("ModeChanged", {
  group = statusgroup,
  pattern = "*:*",
